...
import { Check, CheckCheck, FileText, Paperclip, Pencil, Reply, Send, Trash2, X } from 'lucide-react'
...
export default function MessageThread({ conversationId, currentUserId }: Props) {
  ...
  const loadAttachments = useCallback(async (items: Message[]) => {
    ...
    for (const item of data as Attachment[]) {
      ...
      const attachment = { ...item, url: signed?.signedUrl }
      ;(grouped[item.message_id] ??= []).push(attachment)
    }
  ...
  async function loadOlderMessages() {
    ...
    if (loadError) { setError('Older messages load nahi ho paaye.') } else {
  ...
  async function sendMessage(event: FormEvent<HTMLFormElement>) {
    ...
    if (file) {
      const isImage = IMAGE_TYPES.has(file.type); const allowed = isImage || FILE_TYPES.has(file.type)
  ...
  async function deleteMessage(message: Message) {
    setActionId(message.id); const { error: deleteError } = await supabase.from('messages').update({ deleted_at: new Date().toISOString(), content: null }).eq('id', message.id).eq('sender_id', currentUserId)
    if (deleteError) setError('Message delete nahi ho paaya.'); else setMessages((current) => current.map((m) => m.id === message.id ? { ...m, deleted_at: new Date().toISOString(), content: null } : m)); setActionId(null)
  }
  ...
  return (
    <section style={{ display: 'grid', gap: 12 }}>
      <div ref={scrollRef} onScroll={handleScroll} style={{ minHeight: 420, maxHeight: 620, overflowY: 'auto', border: '1px solid #ddd', borderRadius: 16, padding: 16 }}>
        {loading ? <p>Messages load ho rahe hain…</p> : !messages.length ? <p>Abhi koi message nahi hai. Pehla message bhejo.</p> : <>
          ...
          {messages.map((message) => {
            ...
            return <div key={message.id} style={{ display: 'flex', justifyContent: mine ? 'flex-end' : 'flex-start', marginBottom: 10 }}>
              <div style={{ maxWidth: '82%', minWidth: 80, padding: '10px 12px', borderRadius: 14, background: mine ? '#e8eefc' : '#f3f4f6' }}>
                ...
                {message.deleted_at ? <em>Message deleted</em> : <>{message.content && <div style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{message.content}{message.updated_at !== message.created_at ? <small> (edited)</small> : null}</div>}
      ...
      <form onSubmit={sendMessage} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        <input value={text} onChange={(event) => setText(event.target.value)} placeholder={editing ? 'Message edit karo…' : 'Message likho…'} maxLength={4000} disabled={sending} style={{ flex: 1 }} aria-label="Message" />
