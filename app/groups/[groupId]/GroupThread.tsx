...
export default function GroupThread({ groupId, currentUserId }: { groupId: string; currentUserId: string }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      supabase.from('group_members').select('user_id,profiles(display_name,username)').eq('group_id', groupId)
    ]).then(async ([messageResult, memberResult]) => {
      if (!active) return
  ...
  async function uploadFile(file: File) {
    ...
    const message = data as Message
    const { error: attachmentError } = await supabase.from('group_message_attachments').insert({ message_id: message.id, storage_path: path, file_name: file.name, file_type: file.type, file_size: file.size })
    if (attachmentError) { await supabase.from('group_messages').delete().eq('id', message.id); await supabase.storage.from(bucket).remove([path]); setUploading(false); return }
    setMessages((current) => current.some((m) => m.id === message.id) ? current : [...current, message])
  ...
  return <section style={{ display: 'grid', gap: 10 }}>
    <div style={{ minHeight: 420, maxHeight: 620, overflowY: 'auto', border: '1px solid #ddd', borderRadius: 16, padding: 16 }}>
      {messages.map((message) => <div key={message.id} style={{ marginBottom: 14, textAlign: message.sender_id === currentUserId ? 'right' : 'left' }}>
        ...
        <br /><button type="button" onClick={() => setReplyTo(message)}>Reply</button>
      </div>)}
      <div ref={bottomRef} />
    </div>
